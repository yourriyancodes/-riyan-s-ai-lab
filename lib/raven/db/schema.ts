/**
 * The whole schema, in one string, so `db/schema.sql`, the adapter and the tests can
 * never disagree about what exists.
 *
 * Deliberate omissions: no message content is encrypted at rest here (a portfolio demo
 * on a free Postgres is not the place to invent a crypto story), and nothing secret is
 * ever stored — no API keys, no provider payloads beyond the answer text, no tokens.
 * `tool_runs.input`/`output` are jsonb precisely so a run can be audited later.
 */
...
].join('\n')}\n\n${SCHEMA_STATEMENTS.map((statement) => `${statement};`).join('\n\n')}\n`

export const TABLES = {
  conversations: 'raven_conversations',
  messages: 'raven_messages',
  memories: 'raven_memories',
  agentRuns: 'raven_agent_runs',
  toolRuns: 'raven_tool_runs',
} as const
